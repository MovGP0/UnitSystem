...
public abstract class QsValue: IComparable
{
    ...
    virtual public QsValue PowerDotOperation(QsValue value)
    {
        ...
        var count = Qs.IntegerFromQsValue((QsScalar)value);
    ...
    {
        throw new QsException(string.Format("Execute ({0}) command is not valid for {1}", expression.TokenValue, GetType()));
    }
    ...
    {
        throw new QsException("Exclamation operator is not valid for " + GetType());
    }

    virtual public QsValue ColonOperator(QsValue value)
    {
        throw new QsException("Colon operator is not valid for " + GetType());
    }
    #endregion
    ...
    {
        var vec = new QsVector(values.Length);
    ...
    {
        var value = obj as QsValue;
        if (obj != null)
        {
            if (LessThan(value)) return -1;
            if (Equality(value)) return 0;
            if (GreaterThan(value)) return 1;
            throw new QsException("Comparison failed between " + GetType().Name + " and " + value.GetType().Name);
        }
