...
public class Root
{
    ...
    public static void LoadLibrary(string library)
    {
        ...
        {
            var a = Assembly.LoadFrom(library);
        }
#endif
    }
        ...
        foreach (var p in parameters)
        {
            ...
            else if (p.QsNativeValue is QsScalar)
            {
                ...
                else if(paramInfos[iy].ParameterType.IsGenericType)
                {
                    ...
                    {
                        var nativeValue = Convert.ChangeType(scalar.NumericalQuantity, paramInfos[iy].ParameterType, null);
                        NativeParameters.Add(nativeValue);
                    }
                }
                ...
                {
                    var nativeValue = Convert.ChangeType(scalar.NumericalQuantity.Value, paramInfos[iy].ParameterType, null);
                    NativeParameters.Add(nativeValue);
                }
            }
            ...
            else if (p.QsNativeValue is QsVector)
            {
                ...
                {
                    // target is array
                    var vec = (QsVector)p.QsNativeValue;
                    var ArrayType = Type.GetType(paramInfos[iy].ParameterType.FullName.Trim('[', ']'));
                    var arr = Array.CreateInstance(ArrayType, vec.Count);
                    for (var i = 0; i < vec.Count; i++)
                    {
                        var val = Convert.ChangeType(vec[i].NumericalQuantity.Value, ArrayType, null);
                        arr.SetValue(val, i);
                    }
            ...
            else if (p.QsNativeValue is QsObject)
            {
                ...
                var qso = (QsObject)p.QsNativeValue;
    ...
    public static QsValue? NativeToQsConvert(object? value)
    {
        ...
        {
            var tupleValues = new List<QsTupleValue>();

            foreach(var eval in Enum.GetValues(EnumType))
            {
                var ival = (int)eval;

                var tpv = new QsTupleValue(
                    ival,
                    Enum.GetName(EnumType, eval),
                    ival.ToScalarValue());
        ...
        {
            var vv = (double)Convert.ChangeType(value, typeof(double), null);
            return new QsScalar
            {
    ...
    public static Expression QsToNativeConvert(Type targetType, Expression value)
    {
        ...
        throw new QsException("Couldn't convert the value (" + value.Type + ") expression to the target type (" + targetType + ")");
    }
    ...
    private static Numeric[] ToNumericArray<Numeric>(QsValue value) where Numeric : struct
    {
        ...
        {
            var ListStorage = vector.ToArray();

            var targetType = typeof(Numeric);

            var dd = new Numeric[ListStorage.Length];
            var ix = ListStorage.Length - 1;
            while (ix >= 0)
            {
    ...
    public static object? QsToNativeConvert(Type targetType, object value)
    {
        ...
        if (targetType.IsValueType && value is QsFlowingTuple tuple)
        {
            // there is a probability that we want to set the fields of target value type with the values exist in flowing tuple
            return tuple.FromTuple(targetType);
        }

        throw new QsException("Couldn't convert the value (" + value.GetType() + ") expression to the target type (" + targetType + ")");
    }
}