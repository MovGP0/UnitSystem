...
public partial class SymbolicVariable
{
    ...
    /// </summary>
    public SymbolicVariable? SymbolPowerTerm => _SymbolPowerTerm;
    ...
    {
        var stripped = new SymbolicVariable(Symbol);

        stripped._SymbolPower = _SymbolPower;

        if (_SymbolPowerTerm != null)
            stripped._SymbolPowerTerm = _SymbolPowerTerm.Clone();
    ...
    public SymbolicVariable GetTheStrippedCoefficient()
    {
        ...
        if (_CoeffecientPowerTerm != null)
            stripped._CoeffecientPowerTerm = _CoeffecientPowerTerm.Clone();
    ...
    public bool BaseEquals(SymbolicVariable sv)
    {
        ...
        if (!DividedTerm.Equals(sv.DividedTerm)) return false;

        if (FusedSymbols.Count > 0 || sv.FusedSymbols.Count > 0)
        {
            ...
            foreach (var cv in FusedSymbols)
            {
        ...
        {
            if (Symbol.Equals(sv.Symbol, StringComparison.OrdinalIgnoreCase) && DividedTerm.Equals( sv.DividedTerm))
            {
                if (Symbol == string.Empty) return true;  //because there is no symbol or empty symbol which indicates that the term is coefficient only
                else if (_SymbolPowerTerm != null && sv._SymbolPowerTerm != null)
                {
                    if (_SymbolPowerTerm.Equals(sv._SymbolPowerTerm)) return true;
                }
                else if (SymbolPower != sv.SymbolPower)
                {
                    return false;
                }
                else
                {
                    ...
                    if (SymbolPowerText.Equals(sv.SymbolPowerText, StringComparison.OrdinalIgnoreCase))
                        return true;
                }
    ...
    public bool Equals(SymbolicVariable sv)
    {
        ...
        else
        {
            ...
            if (Symbol.Equals(sv.Symbol, StringComparison.OrdinalIgnoreCase))
            {
                if (SymbolPower == sv.SymbolPower)
                {
                    if (Coeffecient == sv.Coeffecient)
                    {
                        return true;
                    }
                }
    ...
    public bool IsCoeffecientOnly
    {
        get
        {
            if (string.IsNullOrEmpty(WholeValueBaseKey) && IsMultiTerm == false)
            {
                return true;
            }
    ...
    public string[] GetInvolvedSymbols()
    {
        ...
        {
            symbols.AddRange(_BaseVariable.InvolvedSymbols);
        }
    ...
    public SymbolicVariable Clone(bool excludeExtraTerms = false)
    {
        ...
        clone.Coeffecient = Coeffecient;
        clone._SymbolPower = _SymbolPower;

        if (_SymbolPowerTerm != null)
            clone._SymbolPowerTerm = _SymbolPowerTerm.Clone();

        if (_CoeffecientPowerTerm != null)
            clone._CoeffecientPowerTerm = _CoeffecientPowerTerm.Clone();

        if (_BaseVariable != null)
            clone._BaseVariable = _BaseVariable.Clone();
